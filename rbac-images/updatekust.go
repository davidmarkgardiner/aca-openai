...
func main() {
    ...
    for _, f := range files {
        ...
        if filepath.Ext(f.Name()) == ".json" {
            ...
            if _, err := os.Stat("../../environment2/westeurope/kd469b39473-we01/kustomization.yaml"); os.IsNotExist(err) {
                // The file doesn't exist, create it
                _, err := os.Create("../../environment2/westeurope/kd469b39473-we01/kustomization.yaml")
                if err != nil {
                    log.Fatalf("Failed to create file: %v", err)
                }
            }
            // Read the kustomization.yaml file
            data, err = os.ReadFile("../../environment2/westeurope/kd469b39473-we01/" + "kustomization.yaml")
